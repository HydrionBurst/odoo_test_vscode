...
export class Configuration {
    ...
    static async autoSetPaths() {
        ...
        if (firstFolder) {
            dumpPath = path.join(firstFolder.uri.fsPath, ".dumps");
        }
    ...
    static async check(): Promise<void> {
        ...
        if (upgradePath.length === 0) {
            vscode.window.showErrorMessage("At least one valid upgrade path is required");
            hasErrors = true;
        } else {
        ...
        // dump path
        const dumpPath = config.get<string>("dumpPath");
        if (dumpPath) {
            if (!(await Configuration.checkIsDirectory(dumpPath))) {
                vscode.window.showErrorMessage(`Dump path is not a directory: ${dumpPath}`);
                hasErrors = true;
            }
        }
