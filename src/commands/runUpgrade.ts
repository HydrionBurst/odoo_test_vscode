...
export async function upgradeDatabase(module:string, branchName: string, withPrepare: boolean = true) {
    if (!(await isModuleInstalled(module))) {
...
export async function checkUpgradeTest(module: string, className: string, filePath: string) {
    ...
    if (!(await isUpgradeTestPrepared(module, className))) {
        vscode.window.showWarningMessage(`Upgrade test data for ${className} is not prepared in the database`);
        return;
    }
